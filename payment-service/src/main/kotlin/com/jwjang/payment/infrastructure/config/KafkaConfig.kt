package com.jwjang.payment.infrastructure.config
...
class KafkaConfig(
    ...
    @Bean("paymentRequestedFactory")
    fun paymentRequestedContainerFactory(): ConcurrentKafkaListenerContainerFactory<String, PaymentRequestedEvent> {
        val deserializer = JsonDeserializer(PaymentRequestedEvent::class.java).apply {
            addTrustedPackages("com.jwjang.*")
            setUseTypeHeaders(false)
        }
        val consumerFactory: ConsumerFactory<String, PaymentRequestedEvent> =
            DefaultKafkaConsumerFactory(baseConsumerProps(), StringDeserializer(), deserializer)
        return ConcurrentKafkaListenerContainerFactory<String, PaymentRequestedEvent>().apply {
